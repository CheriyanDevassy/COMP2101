...
#    - change eno1 to whatever in terface you have and want to gather info about in order to test the script
...
lan_address="$(hostname -I)"
lan_name=$(getent hosts $lan_address | awk '{print $2}')
...
c=$(lshw -class network | awk '/logical name:/{print $3}' | wc -l)

for((w=1;w<=$c;w+=1));
do interface=$(lshw -class network | awk '/logical name:/{print $3}' | awk -v z=$w 'NR==z{print $1; exit}')
if [[ $interface = lo* ]] ;
then continue ; fi
